...
import { EPISODE_START_IDS, EPISODE_START_LABEL, EpisodeStartId } from '@painlink/shared';
import { View } from 'react-native';
import { SelectablePill } from '@/components/ui/selectable-pill';

export const EpisodeStartField = ({ form }: { form: OpenPainSpikeForm }) => {
  return (
    <FormControlField<OpenPainSpikeFormValues, EpisodeStartId>
      control={form.control}
      name="episodeStart"
      render={({ value, onChange }) => (
        <View className="flex-row flex-wrap gap-3 mt-1">
          {EPISODE_START_IDS.map((id) => (
            <SelectablePill
              key={id}
